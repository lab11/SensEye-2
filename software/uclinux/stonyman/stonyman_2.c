...
#define REG_CAM_REG_SPACE_WIDTH         (0x00000010ul)
...
#define REG_OFFSET_GLOB_RESET           (0x00000004ul)
#define REG_OFFSET_GLOB_TIMING          (0x00000008ul)
...
#define REG_OFFSET_CAMX_FRAMEMASK       (0x00000000ul)
#define REG_OFFSET_CAMX_SETTINGS1       (0x00000004ul)
#define REG_OFFSET_CAMX_SETTINGS2       (0x00000008ul)

#define REG_CTRL                        \
   (*((volatile uint32*)(REG_BASE_ADDR_CAM+REG_OFFSET_GLOB_STARTCAP)))
#define REG_RESET                       \
   (*((volatile uint32*)(REG_BASE_ADDR_CAM+REG_OFFSET_GLOB_RESET)))
#define REG_TIMING                      \
   (*((volatile uint32*)(REG_BASE_ADDR_CAM+REG_OFFSET_GLOB_TIMING)))
...
#define REG_CAMX_FRAMEMASK(xx)             \
   (*((volatile uint32*)(REG_CAMX_BASE(xx)+REG_OFFSET_CAMX_FRAMEMASK)))
#define REG_CAMX_SETTINGS1(xx)             \
   (*((volatile uint32*)(REG_CAMX_BASE(xx)+REG_OFFSET_CAMX_SETTINGS1)))
#define REG_CAMX_SETTINGS2(xx)             \
   (*((volatile uint32*)(REG_CAMX_BASE(xx)+REG_OFFSET_CAMX_SETTINGS2)))
...
//printk(KERN_INFO "\n\tstonyman_read: head_idx=%d\ttail_idx=%d\thead_bufpos=%d\ttail_bufpos=%d\n", g_img_buf_head_idx[0], g_img_buf_tail_idx[0], g_img_buf_head_bufpos[0], g_img_buf_tail_bufpos[0]);
...
irqreturn_t stonyman_interrupt(int irq, void *dev_id, struct pt_regs *regs)
{
    ...
    static char vref = 41;
    static char nbias = 49;
    static char aobias = 46;
   ...
   if(RESOLUTION == g_img_buf_tail_bufpos[0])
   {
          ...
          (((g_img_buf_tail_idx[0]+1) % IMG_BUF_QUEUE_LEN) != g_img_buf_head_idx[0]) )
      {
         ...
         // Apply new settings
         REG_CAMX_SETTINGS1(0) = ((vref<<24)|((17)<<16)|(nbias<<8)|(aobias));
         REG_RESET = 1u;
         counter++;
         if (counter == 30) {
            counter = 0;
            ...
            //nbias++;
            //if (nbias == 55) nbias = 40;

            //aobias++;
            //if (aobias == 55) aobias = 40;
            printk(KERN_INFO "Vref=%d\tNbias=%d\tAobias=%d", vref, nbias, aobias);
         }

         // Wait until completed
         //while (0x01&REG_FLAGS);
